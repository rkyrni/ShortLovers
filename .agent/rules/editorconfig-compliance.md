...
Follow the formatting rules defined in `.editorconfig` for all code that is created or modified.
Ensure consistent code style, indentation, line endings, character encoding, and whitespace handling
across all files. Maintain adherence to the configuration file's specifications including but not
limited to tab width, indent style, end of line characters, charset encoding, trailing whitespace
handling, and insert final newline requirements. Verify that all code modifications comply with the
established `.editorconfig` standards to ensure code uniformity and readability throughout the
project.

Here is the .editorconfig value:
